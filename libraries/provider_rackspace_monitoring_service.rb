  ...
  class Provider
    ...
    class RackspaceMonitoringService < Chef::Provider::LWRPBase
      ...
      action :create do
        ...
        auto_create =  new_resource.create_entity ? '--auto-create-entity' : ''

        # Set up rackspace-monitoring-agent
        execute 'agent-setup-cloud' do
          command "rackspace-monitoring-agent --setup --username #{parsed_cloud_credentials_username} --apikey #{parsed_cloud_credentials_api_key} #{auto_create}"
          action 'run'
